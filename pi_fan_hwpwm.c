...
void PWM_out(int level) {
  ...
  if(level != pwm_level) {
    int outlevel = invert != 0 ? MAX_FAN - level : level;
    if(pin == 0 || pin == 13) bcm2835_pwm_set_data(1, outlevel);
    if(pin == 0 || pin == 18) bcm2835_pwm_set_data(0, outlevel);
    pwm_level = level;
  }
}
